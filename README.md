...
6. `bash test/run.sh && bash test/hook.sh && bash test/preflight.sh && bash test/e2e.sh && bash test/e2e-orca.sh && bash test/defaults-doctor.sh && bash test/board-relay.sh && bash test/review-drive.sh && bash test/setup-plumbing.sh && bash test/setup-defaults.sh && bash test/setup-pboard.sh && bash test/setup-target.sh && bash test/setup-safety.sh` — all must pass.
...
`./drive.sh setup` is the automated form of the one-time checklist above — an idempotent,
overwrite-safe install/config wizard for the trio. It is a peer of `doctor`
...
it cannot automate prints the exact `fix:` remediation and counts as blocking. It generates
ONLY the three hermetic config artifacts (ADR 0004): `drive.defaults`, the `pboard()` shell
block, and a target repo's `.pipeline/roles.yaml`.
...
- **Skip a step:** `SETUP_DO_<STEP>=0` for any of `PBOARD DEFAULTS TARGET DOCTOR`
  (default `1` for each). Run a no-op dry check with
  `SETUP_YES=1 SETUP_DO_PBOARD=0 SETUP_DO_DEFAULTS=0 SETUP_DO_TARGET=0 SETUP_DO_DOCTOR=1 ./drive.sh setup`.
- **Idempotent:** generated config (`drive.defaults`, a target repo's `.pipeline/roles.yaml`)
  ...
  `# <<< pipeline pboard <<<` and replaces only its own marked region (it refuses a malformed or mis-ordered marker layout,
  and writes via an atomic same-dir temp + rename that preserves the original mode; it never
  follows a symlinked destination).
- **What it never does:** declare success on its own, follow a symlinked config/rc destination, interpret a prompt EOF as consent,
  or scaffold a per-feature `drive.config`
