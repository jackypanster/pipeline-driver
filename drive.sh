...
while : ; do
  ...
  # ("re-run to resume") loop back into the same halt forever. The exit is the same
  # read-then-bind ritual as GATE 1, per invocation and in-process only: type the
  # rejection seq to confirm the review was read, and the loop proceeds to dispatch the
  # fix; EOF or a mismatch preserves the historical halt. A NEW rejection (different
  # seq) re-fires the gate.
  if [ "${FROM:-}" = "review" ] && [ "$NEXT" = "impl" ] && [ "${ACKED_REJECTION_SEQ:-}" != "$SEQ" ]; then
    ...
    if [ "$YOLO" = "1" ]; then
      note "YOLO=1 — the standing grant extends to this ack: the coordinating agent may"
      note "type the seq below AFTER reading the review. Merge confirm stays human."
    fi
