...
# exception to "no scheduler" (README §For agents; the first is drive.sh's impl loop).
#
...
#                       reviewed-head: <the 40-char sha it reviewed>
#                       findings: <count>
...
note "gates   : the loop never merges; 'verdict: approved' halts for YOUR merge confirm"
...
while : ; do
  ...
  send_to "$REVIEWER" "Review PR $PR_URL at head $head. Read the full diff and every earlier round's comments via gh, verify the claims hold, then post EXACTLY ONE PR comment (gh pr comment) whose first three lines are: 'verdict: approved' or 'verdict: changes-requested', then 'reviewed-head: $head', then 'findings: <count>' — followed by each finding with file:line and a concrete failure scenario. Never merge, never close the PR, never push, never use GitHub review approvals: the comment IS the verdict." \
...
EOF
  ...
  # The echo may be a short sha — the round head must START with it (never vice versa).
  case "$head" in "$rhead"*) : ;; *)
...
"
  ...
  send_to "$FIXER" "${hunt}PR $PR_URL at head $head got 'verdict: changes-requested' — read the review comment $curl_ and the full thread via gh. Fix EVERY finding on the PR branch '$HEAD_REF' ONLY: no merge, no rebase, no force-push, never touch '$BASE_REF'. Push your commits, then post EXACTLY ONE PR comment (gh pr comment) whose first line is 'fixed: <new head sha>' followed by per-finding evidence (what changed, how you verified)." \
