...
| no journal progress within `CARD_TIMEOUT` (herdr transport) | the TUI stalled — permission prompt, crash, wedged run (the driver prints the pane tail) | inspect the impl pane in Herdr; manual `pipeline-impl` or `pipeline-hunt` |
